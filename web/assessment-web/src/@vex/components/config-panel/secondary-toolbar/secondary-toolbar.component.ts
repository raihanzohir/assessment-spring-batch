...
@Component({
  ...
  template: `
    ...
    <div [ngClass]="{ 'fixed': fixed$ | async, 'w-full': !(fixed$ | async) }" class="secondary-toolbar shadow-b z-40 border-t flex">
      <div class="px-gutter flex items-center flex-auto" [class.container]="isVerticalLayout$ | async">
        <h1 *ngIf="current"
            class="subheading-2 font-medium m-0 ltr:pr-3 rtl:pl-3 ltr:border-r rtl:border-l ltr:mr-3 rtl:ml-3 flex-none border-r-2 border-[#dadadb]">{{ current }}</h1>
